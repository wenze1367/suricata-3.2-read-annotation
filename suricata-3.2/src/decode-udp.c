...
DecodeUDP继续执行，调用DecodeTeredo尝试进行Teredo隧道解码，见RFC 4380：
Teredo: Tunneling IPv6 over UDP through Network Address Translations (NATs)。
若解码成功，即这是一个Teredo隧道包，则直接调用FlowHandlePacket，并返回，不再进行应用层处理
（因为已经知道这个包的用途了？）。
否则，在调用FlowHandlePacket后，再调用AppLayerHandleUdp对该UDP包的应用层数据进行进一步处理。

为什么解码TCP时没有调类似的AppLayerHandle函数呢？ 其实有调用，但却是在StreamTCP模块，
即对于TCP包的应用层分析是基于重组后的包。
*/
