...
static void *TmThreadsManagement(void *td)
{
    ...
    /* Drop the capabilities for this thread ;什么都不做*/
    SCDropCaps(tv);
	...
	/*这个函数比较难懂，首先调用StatsGetAllCountersArray，
	将(tv)->perf_private_ctx中的stats的counter指向所有的(tv)->perf_public_ctx，
	我理解这里是会记到stats.log中的统计内容的列表，针对不同的tv会有所不同，
	在流管理中为如下几个统计：“flow_mgr.closed_pruned”“flow_mgr.new_pruned”“flow_mgr.est_pruned”“flow.spare”“flow.emerg_mode_entered”“flow.emerg_mode_over”“flow.tcp_reuse”。
	然后调用StatsThreadRegister将每个线程的stats信息存储到stats_ctx->sts中。
	*/
    StatsSetupPrivate(tv);
...
-------+++++++++++
 注：目前Suricata中关于packet pool的部分比较乱。例如，虽然包获取线程的inq和inqh都设为”packetpool”，
 但首先来说，对于包获取线程inq是没意义的，因此这里这么设置给程序中带来很多特殊处理
 （TmThreadCreate），我认为设为NULL其实更好理解；其次，inqh这么设可能本意是
 想统一包获取过程，即不区分抓包或者从队列入包，都统一从tmqh_in来得到包，
 然而实际上Suricata中却做了特殊区分，对于包获取线程其slot类型为”pktacqloop”，
 相应的执行函数TmThreadsSlotPktAcqLoop并没有调用tmqh_in，
 而是调用其第一个slot中模块的PktAcqLoop。另外，TmqhOutputPacketpool函数
 作为”packetpool”这个handler的OutHandler，本应该只能由线程的tmqh_out来调用，
 但实际上程序中却到处都有调用（主要目的是跳过后续的线程处理，直接返回到数据包池中）。
 总之，这块是我目前感觉到Suricata中最ugly的。。。
 由于autofp是目前几乎全部运行模式类型下的默认模式（参见RunModeDispatch），
 下面将重点介绍”flow”队列handler的实现。
 */
