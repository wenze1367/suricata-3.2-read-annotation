...
/*
*FlowWorker -> StreamTcp -> StreamTcpPacket->StreamTcpReassembleHandleSegment;
*(1)IDS:首先处理对端stream已经缓存的数据包，进行应用层协议识别。函数栈为StreamTcpReassembleHandleSegmentUpdateACK -> StreamTcpReassembleAppLayer -> ReassembleUpdateAppLayer。
然后处理当前packet的数据，加入到本端stream的缓存中。函数为StreamTcpReassembleHandleSegmentHandleData。
*
*
*(2)IPS:首先处理当前packet的数据，加入到本端stream的缓存中。函数为StreamTcpReassembleHandleSegmentHandleData。
