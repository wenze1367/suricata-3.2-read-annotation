...
总结：
suricata会在分片数据包被重组完成后才会调用后续的处理流程解传输层的头信息。
包重组实在receive线程完成的，这一过程对后续的worker线程是透明的。
	*/
    if (!more_frags) {
        tracker->seen_last = 1;
    }
 ...
 *     NULL is returned.
 tracker为申请的一块内存，我理解是用来存放分片包的，
 tracker里面会保存一个原始IP包的所有分片，直到这些分片被重组成功。
 DefragInsertFrag这个函数将新收到的p插入刚刚得到的tracker，如果p是最后一个分片，
 那么会返回一个重组后的packet结构的指针rp，否则返回NULL。
